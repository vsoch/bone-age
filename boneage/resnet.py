...
class Resnet50:
    ...
    def predict(self, image, is_male):
        assert image.dtype == 'uint8'
        assert image.shape == (256,256,3), image.shape
        scores = self.sess.run(self.pred, feed_dict={self.x : image})
        scores = np.sum(scores, axis=0)
        scores = scores[NUM_AGES::] if is_male else scores[0:NUM_AGES]
...
def fixed_crops(image):
    with tf.name_scope('input'):
        ...
        # Extract ten fixed crops from image
        assert image.get_shape() == (256,256,3)
        height = 256
        width = 256
        target_height = 224
        target_width = 224
        ul = tf.image.crop_to_bounding_box(image,0,0,target_height,target_width)
        ur = tf.image.crop_to_bounding_box(image,0,width-target_width,
                target_height,target_width)
        ll = tf.image.crop_to_bounding_box(image,height-target_height,0,
                target_height,target_width)
        lr = tf.image.crop_to_bounding_box(image,height-target_height,
                width-target_width,target_height,target_width)
        central = tf.image.crop_to_bounding_box(image,
                int((height-target_height)/2.0),
                int((width-target_width)/2.0),
                target_height, target_width)
        flipped_image = tf.image.flip_left_right(image)
        flul = tf.image.crop_to_bounding_box(flipped_image,0,0,
                target_height,target_width)
        flur = tf.image.crop_to_bounding_box(flipped_image,0,width-target_width,
                target_height,target_width)
        flll = tf.image.crop_to_bounding_box(flipped_image,height-target_height,0,
                target_height,target_width)
        fllr = tf.image.crop_to_bounding_box(flipped_image,height-target_height,
                width-target_width,target_height,target_width)
        flcentral = tf.image.crop_to_bounding_box(flipped_image,
                int((height-target_height)/2.0),
                int((width-target_width)/2.0),
                target_height, target_width)
        fixed_crops = [ul, ur, ll, lr, central, flul, flur, flll, fllr, flcentral]
